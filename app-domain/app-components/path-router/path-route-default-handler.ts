...
import type { ParamsType } from './path-route';
...
export const isPathRouteHandlerExecutionResult = (
  result: string | PathRouteHandlerExecutionResult | null,
): result is PathRouteHandlerExecutionResult => result !== null && typeof result !== 'string';

export interface IPathRouteHandler {
  execute(methodParams: HandlerParamsType): PathRouteHandlerReturnType;
}

export type PathRouteHandlerFunctionType = (
  methodParams: HandlerParamsType,
...
export const isPathRouteHandlerFunction = (
  candidate: PathRouteHandlerType,
): candidate is PathRouteHandlerFunctionType => typeof candidate === 'function';
...
export class PathRouteDefaultHandler implements IPathRouteHandler {
  ...
  async _instantiate(
    module: any,
    className: string,
    constructorParams: Array<any> | undefined = undefined,
  ) {
    ...
    return constructorParams ? new targetClass(...constructorParams) : new targetClass();
  }
  ...
  async _executeMethod(instance: any, methodName: string, params: HandlerParamsType) {
    ...
    return instance[normalizedMethodName](params) as Promise<
      PathRouteHandlerExecutionResult | string | null
    >;
  }
