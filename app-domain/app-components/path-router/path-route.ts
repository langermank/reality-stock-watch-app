...
export const isStringPartType = (
  parsedPart: ParsedPathPartType,
): parsedPart is StringParsedPathPartType => parsedPart.type === 'string';
export const isPlaceholderPartType = (
  parsedPart: ParsedPathPartType,
): parsedPart is PlaceholderParsedPathPartType => parsedPart.type === 'placeholder';
export const isGreedyPartType = (
  parsedPart: ParsedPathPartType,
): parsedPart is GreedyPlaceholderParsedPathPartType => parsedPart.type === 'greedy';
...
export class PathRoute {
  ...
  processPathTemplate() {
    const parsedPaths: Array<ParsedPathPartType> = [];
  ...
  match(givenPath: string): ParamsType | null {
    ...
    for (const parsedPart of this.parsedPathTemplate) {
      ...
      if (isPlaceholderPartType(parsedPart)) {
        const givenPart = givenPathParts.shift() || this.defaults[parsedPart.identifier] || '';
  ...
  _validateConstraintDefinitions(
    parsedPathTemplate: Readonly<Array<ParsedPathPartType>>,
    constraints: ConstraintType,
  ) {
  ...
  _matchPlaceholder(part: string, parsedPart: PlaceholderParsedPathPartType) {
    ...
    const identifierPart = part.substring(
      parsedPart.prefix.length,
      part.length - parsedPart.suffix.length,
    );
